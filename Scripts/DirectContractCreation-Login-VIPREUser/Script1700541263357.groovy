...
String AccountName = 'QA_TestAccount_Reseller'+'_'+GlobalVariable.VIPRE_SalesTerritory+'_'+formattedDate
...
WebUI.sendKeys(findTestObject('Object Repository/QA Sandbox/HomePage/input_Searchbox'), 'QA_TestAccount_Reseller', FailureHandling.STOP_ON_FAILURE)
...
CustomKeywords.'vipreQA.CommonMethods.AccountCreation'(GlobalVariable.VIPRE_AccountType,GlobalVariable.AccountName,GlobalVariable.BillingCountry, GlobalVariable.BillingStreet,GlobalVariable.BillingCity,GlobalVariable.PostalCode,GlobalVariable.RegNo,GlobalVariable.VIPRE_SalesTerritory)
URL = WebUI.getUrl()
println (URL)
...
WebUI.comment('Login as VIPRE User')
CustomKeywords.'vipreQA.CommonMethods.LoginasUser'(GlobalVariable.VIPRE_User)
WebUI.switchToWindowIndex(0)
WebUI.comment('Verify the App Element')
WebUI.verifyElementPresent(findTestObject('Object Repository/QA Sandbox/HomePage/App_Verify_VIPRE'), 30)
...
String PrimaryContact = GlobalVariable.FirstName+' '+GlobalVariable.LastName
...
WebUI.comment('Quote Creation')
CustomKeywords.'vipreQA.CommonMethods.QuoteCreation'(GlobalVariable.VIPRE_SalesTerritory, GlobalVariable.VIPRE_BillingEntity, GlobalVariable.VIPRE_BillingModel, GlobalVariable.DirectQuoteType, Acc_Name, PrimaryContact,GlobalVariable.Direct_SalesChannel, GlobalVariable.Declared_BillingTerm)

WebUI.comment('Add Product - Edit Quote Line')
CustomKeywords.'vipreQA.CommonMethods.EditQuoteLine'(GlobalVariable.ProductCode, GlobalVariable.ProductQuantity)
String QuoteURL = WebUI.getUrl()

WebUI.comment('Logout as User')
CustomKeywords.'vipreQA.CommonMethods.LogoutUser'(QuoteURL)

WebUI.comment('Enable Override Signed Document')
CustomKeywords.'vipreQA.CommonMethods.QuoteSigning'()
...
WebUI.navigateToUrl(QuoteURL, FailureHandling.STOP_ON_FAILURE)

WebUI.comment('Accept the Quote before entering Opportunity Reason and Account Primary Billing Contact')
CustomKeywords.'vipreQA.CommonMethods.AcceptingQuote'(GlobalVariable.ReasonWonLost, GlobalVariable.DescReasonWonLost)